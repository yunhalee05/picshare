...
export const createProduct = (product,bodyFormData) =>async(dispatch, getState)=>{
    ...
    try{
        await Axios.post('/api/uploads', bodyFormData,{
            ...
            product = {...product, image:res.data}
...
export const updateProduct = (product, productImage)=>async(dispatch, getState)=>{
    dispatch({
...
export const getTopProducts = () => async (dispatch) => {
    ...
    try {
      const res  = await Axios.get('/api/products/products/top-products');
      dispatch({ type: GET_TOP_PRODUCTS_SUCCESS});
      return res.data

    } catch (error) {
      dispatch({ type: GET_TOP_PRODUCTS_FAIL, payload: error.message });
    }
